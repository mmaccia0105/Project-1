...
county_data_test <- process_census_data(url = "https://www4.stat.ncsu.edu/~online/datasets/EDU01a.csv",  survey_stat_name = "enrollment_count")
state_data_test <- process_census_data(url = "https://www4.stat.ncsu.edu/~online/datasets/EDU01b.csv", survey_stat_name = "enrollment_count")
...
plot.state(census_test[[2]],var_name = "enrollment_count")
```

Next we are going to create a plotting function for the `county` level data. The goal of this function will be for the user to be able to input a state of interest using the 2-letter abbreviated (the default value is NC), determine whether the `top` or `bottom` most counties are the target (default of `top`), and finally an instruction to determine how many of the 'top' or bottom most counties should be investigated (default of 5).
...
plot.county(census_test[[1]],var_name = "enrollment_count", 
                        state.name="WA", 
                        top_bottom = "bottom", 
                        n_counties = 4)
...
county_data <- process_census_data(url = "https://www4.stat.ncsu.edu/~online/datasets/EDU01a.csv",  survey_stat_name = "enrollment_count")
state_data <- process_census_data(url = "https://www4.stat.ncsu.edu/~online/datasets/EDU01b.csv", survey_stat_name = "enrollment_count")
...
data_pst01a <- process_census_data(url = "http://www4.stat.ncsu.edu/~online/datasets/PST01a.csv",  survey_stat_name = "enrollment_count")

data_pst01b <- process_census_data(url = "http://www4.stat.ncsu.edu/~online/datasets/PST01b.csv",  survey_stat_name = "enrollment_count")

data_pst01c <- process_census_data(url = "http://www4.stat.ncsu.edu/~online/datasets/PST01c.csv",  survey_stat_name = "enrollment_count")

data_pst01d <- process_census_data(url = "http://www4.stat.ncsu.edu/~online/datasets/PST01d.csv",  survey_stat_name = "enrollment_count")

#now we need to run our data combining function 3 times to get our desired list that contains 2
#tibbles, 1 for county data and 1 for state data
...
```{r}
plot.state(census_pst_abcd[[2]],var_name = "enrollment_count")
...
```{r}
plot.county(census_pst_abcd[[1]],var_name = "enrollment_count", 
                        state.name="CA", 
                        top_bottom = "top", 
                        n_counties = 15)
```
